...
// The code asks for a button on input D3 to cycle between envelope modes.
...
void setup() {  
  ...
  invState = digitalRead(invertPin);
...
switch (scan){
  ...
  break;  
  case 5:                                // read the envelope invertion mode select button
  if(digitalRead(invertPin)!=invState){
    invState = !invState;
    if(invState == LOW){
      invert_mode++;
      if (invert_mode>2){
