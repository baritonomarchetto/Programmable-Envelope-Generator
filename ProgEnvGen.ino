// GIP1 - Programmable Envelope Generator
//
// a simple ADSR for the Arduino
//
// based on that brilliant piece of code by m0xpd, Feb 2017
//
// see http://m0xpd.blogspot.co.uk/2017/02/signal-processing-on-arduino.html
//
// MODs to the original code
// - pin definitions are now compatible with 492X library
// - biased semi-inverted envelope mode added. The code asks for a latching switch on input D3, like the already coded loop mode (D2).
...
void setup() {  
  ...
  pinMode(gatePin, INPUT);          //this is set as INPUT and pulled to ground with a pulldown resistor (needed to have the trigger switch working as it should)
...
void loop() {
    ...
    while(trigger){  
      ...
      if(loop_mode==true && decay==true){              // in loop mode, break out at the end of the decay
        if(invert_mode == false && envelope<(float)(sustain_Level+1.0)){ 
          decay = false;
          break;
        }
        else if(invert_mode && envelope>(float)(sustain_Level-1.0)){ 
          decay = false;
          break;
        }
      }
...
void update_params(int scan){             // read the input parameters
  ...
  break;  
  case 5:                                 // read the envelope invertion mode input
  invert_mode = !digitalRead(invertPin);
  break;  
  }
}
